...
class NewDashboardController extends BaseController
{
    ...
    { 
        ...
        if(auth()->user()->hasRole('user'))
        {
            $check_in_count = $checkin_query->where('user_id',auth()->user()->id)->whereDate('checkin_date', '>=', $sixMonthsAgo)->count();
            $room_count = $query->where('user_id',auth()->user()->id)->whereDate('checkin_date', '>=', $sixMonthsAgo)->count();
            $party_count = $party_query->where('user_id',auth()->user()->id)->whereDate('checkin_date', '>=', $sixMonthsAgo)->count();
            $sports_count = $sports_query->where('user_id',auth()->user()->id)->whereDate('checkin_date', '>=', $sixMonthsAgo)->count();
        }
        else{
        ...
        // dd($month_earnings[0]);
         ...
         { 
           ...
           return response()->json($data_array);
            ...
            $pending_approval = RoomBooking::where('is_admin_approve',0)->count(); 
            return view('admin.new_dashboard', compact('page', 'main_menu','currency', 'sub_menu','data', 'complaint','sub_menu_1','room_count','party_count','sports_count','pending_users','recent_booking_datas','recent_booking_party','recent_booking_sports','month_earnings','status','check_in_count','pending_approval'));
         } 
   }
    ...
    public function export(httprequest $request)
    {
        ...
        switch($type)
        {
            case 1:  
                if(auth()->user()->hasRole("user"))
                {
                    $booking = RoomBooking::where('user_id',auth()->user()->id);
                    $party = PartyBooking::where('user_id',auth()->user()->id);
                    $sports = SportsBooking::where('user_id',auth()->user()->id);
                }
                else{
        ...
        { 
            ...
            switch ($date_value) {
                case 1: 
                    ...
                    $query->whereBetween('checkin_date', [$fromDate, $toDate]);  
                    $formattedFromDate = $fromDate->isoFormat('Do MMMM YYYY');
                    $formattedToDate = $toDate->isoFormat('Do MMMM YYYY');
                    if(auth()->user()->hasRole('user'))
                    {
                        $title = "Bookings ".$formattedFromDate." - ".$formattedToDate;
                    }
                    else{
                        $title = "Earnings ".$formattedFromDate." - ".$formattedToDate;
                    }
            ...
            if(auth()->user()->hasRole('user'))
            {
                $query->whereDate('checkin_date', '>=', $sixMonthsAgo);
            }
            else{
                $query->whereDate('checkin_date', $today);
            }  
