 ...
 * vechicle types Apis
 */
class SportsBookingController extends BaseController
{
    /**
    ...
    */
    ...
    //    dd("sdvsdfs");
        ...
        //     $title = 'Pending Lawn Booking';
        // }
        // if($request->type == "checkin")
        // {
        //     $sub_menu = 'checkin-booking';
        //     $title = 'Checkin Lawn Booking';
        // }
        // if($request->type == "cancelled")
        // {
        //     $sub_menu = 'cancelled-booking';
        //     $title = 'Cancelled Lawn Booking';
        // }
        // if($request->type == "completed")
        // {
        //     $sub_menu = 'completed-booking';
        //     $title = 'Completed Lawn Booking';
        // }
        $sub_menu_1 = '';
        $user = $this->user->belongsTorole(Role::USER)->get(); 
        return view('admin.sports_booking.index', compact('page', 'main_menu', 'sub_menu','sub_menu_1','user'))->render();
    }
    ...
    { 
        ...
        if(auth()->user()->hasRole('super-user') || auth()->user()->hasRole('mess-manager'))
        {
            ...
            $query = SportsBooking::where('user_id',auth()->user()->id);
        }
        ...
        return view('admin.sports_booking._types', compact('results'))->render();
    }
    ...
    */
    ...
    { 
       ...
       $date_diff = $checkinDate->day - $currentDate->day;
        ...
        return view('admin.sports_booking.view', compact('page', 'main_menu', 'sub_menu','sub_menu_1','booking','date_diff'))->render();
    }
    /**
    * Get Types by admin for ajax
    *
    */
    public function book_now(Request $request)
    {
        ...
        if(isset($request->id))
        {
            $totalDaysBooked = SportsBooking::whereBetween('checkin_date', [$startOfMonth, $endOfMonth])
        ...
        if(isset($request->id))
        {
            $bookings =  SportsBooking::find($request->id); 
        }
        else{
    ...
    */
    public function check_availability(Request $request)
    {
        ...
        if(isset($request->id))
        {
            $bookings = SportsBooking::where(function($query) use ($request) {
            ...
            $bookings = SportsBooking::whereBetween('checkin_date', [$request->from_date, $request->to_date])->orwhereBetween('checkout_date', [$request->from_date, $request->to_date])
                                ->whereIn('status',[0,1]) 
                                ->get();
        } 
        // if(count($bookings) == 0)
    ...
    *
    */
    public function edit(SportsBooking $booking)
    {   
        // dd($booking);
        ...
        $sub_menu_1 = '';
        $user = $this->user->belongsTorole(Role::USER)->get(); 
        return view('admin.sports_booking.update', compact('page','main_menu', 'sub_menu','sub_menu_1','user','booking'));
    }
    ...
    public function export(SportsBooking $booking)
    {     
        ...
        // print_r($data);
        // exit;
        $pdf = PDF::loadView('pdf.sports.invoice', $data);  
        return $pdf->download('sports_invoive_'.time().'.pdf');
    }
    public function test(SportsBooking $booking)
    {     
        $page = trans('pages_names.types');
        $main_menu = 'master';
        $sub_menu = 'types';
        ...
        return view('pdf.sports.test',compact('booking','invoice','user_details','page','main_menu','sub_menu'));
    }
}
