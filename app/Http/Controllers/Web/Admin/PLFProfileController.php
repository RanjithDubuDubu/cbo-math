...
class PLFProfileController extends BaseController
{
    /**
    ...
    */
    public function get_shg_data(Request $request)
    {
        $get_habitation = Habitation::find($request->data);
        // dd($get_habitation);
        $mapped_shg_data = Shg::where('is_completed',1)->where('approved',1)->where('is_plf_mapped',1)->where('habitation',$get_habitation->habitation)->get();

        $unmapped_shg_data = Shg::where('is_completed',1)->where('is_plf_mapped',0)->where('habitation',$get_habitation->habitation)->get();
        return response()->json(['status'=>true,"mapped_shg_data"=>$mapped_shg_data,"unmapped_shg_data"=>$unmapped_shg_data]);
    }
     /**
    * Get all users
    * @return \Illuminate\Http\JsonResponse
    */
    public function get_shg_datas(Request $request)
    {
        $get_habitation = Habitation::find($request->data);
        // dd($get_habitation);
        $mapped_shg_data = Shg::where('is_completed',1)->where('approved',1)->where('is_plf_mapped',1)->where('habitation',$get_habitation->habitation)->get(); 
        return response()->json(['status'=>true,"mapped_shg_data"=>$mapped_shg_data]);
    }
    ...
    * @return \Illuminate\Http\JsonResponse
    */
    public function get_ec(Request $request)
    {
        $get_ec_data = Ec::where('plf_id',$request->plf_id)->get();
        // dd($get_ec_data);   
       return response()->json(['status'=>true,"get_ec_data"=>$get_ec_data]);
    }
    ...
    * @return \Illuminate\Http\JsonResponse
    */
    public function get_shg_member_datas(Request $request)
    {
        // $get_habitation = Shg::find($request->data);
        // dd($get_habitation);
        $mapped_shg_data = Shgmember::where('is_active',1)->where('shg_id',$request->data)->get(); 
        return response()->json(['status'=>true,"mapped_shg_data"=>$mapped_shg_data]);
    }
    /**
    * Get all users
    * @return \Illuminate\Http\JsonResponse
    */
    public function map_shg_data(Request $request)
    { 
        // dd($request->all());
        if($request->data_val == "unmap")
        {
            foreach($request->data as $value)
            {
                // dd($value);
                $shg_data = Shg::find($value);
                $shg_data->is_plf_mapped = 0;
                $shg_data->save();
            }
        }
        else{
            ...
            {
                $shg_data = Shg::find($value);
                $shg_data->is_plf_mapped = 1;
                $shg_data->save();
            }
        }
        
         
        return response()->json(['status'=>true,"message"=>"mapping Done Successfully"]);
    }
    ...
    */
    public function index()
    {
        ...
        $main_menu = 'plf_profile';
        $sub_menu = 'plf_profile';
        $sub_menu_1 = 'plf_profile';
        ...
        $check_plf_profiles = PlfProfile::where('block',$get_panchat->id)->get(); 
        if(count($check_plf_profiles) == 0)
        { 
            foreach($get_panchat->BlockDetails as $k=>$v)
            {
                $plf_profile = new PlfProfile();
                $plf_profile->block = $get_panchat->id;
                $plf_profile->panchayat = $v->id;
                $vo_id = $this->get_generation($v->villagecode,$user_data);
        ...
        return view('admin.plfprofiles.index', compact('page', 'main_menu', 'sub_menu','sub_menu_1'));
    }
    ...
    */
    public function view(User $user)
    {
        ...
        return view('admin.plfprofiles.view', compact('page', 'main_menu', 'sub_menu','sub_menu_1','user','membership_tariff'));
    }
        /**
    * Get all users
    * @return \Illuminate\Http\JsonResponse
    */
    public function viewBankProfile(PlfProfile $profile)
    { 
         // dd("fgdfgfdgdfgdf");
         $page = trans('pages_names.users');
    ...
    public function getbank_date(PlfProfile $profile,QueryFilterContract $queryFilter)
    {
        $url = request()->fullUrl(); //get full url
        $user_data = session('user_details');
        $get_panchat = Blocks::where('blockname',$user_data['block_name'])->first(); 
        $check_plf_profiles = PlfProfile::where('block',$get_panchat->id)->get(); 
        $query = PlfBankProfile::where('plf_id',$profile->id);
    ...
    * @return \Illuminate\Http\JsonResponse
    */
    public function profilesave(PlfProfile $profile,$stage,Request $request)
    { 
        // dd($request->all());
        if($request)
        {
            ...
            case 3: 
                ...
                case 4 :
                    OfficeBearers::where('plf_id',$request->plf_id)->delete();
                    // dd($request->all());
                    if($request->president)
                    {
                        $office_bearer = new OfficeBearers(); 
                        $office_bearer->plf_id = $request->plf_id;
                    ...
                    case 6: 
                        ...
                        if($request->bank_id)
                        {
                            $check_already_fund_exists = PlfBankProfile::where('plf_id',$request->plf_id)->where('acc_type',$request->AccountType)->where('id','!=',$request->bank_id)->get();
                        }
                        else{
                            $check_already_fund_exists = PlfBankProfile::where('plf_id',$request->plf_id)->where('acc_type',$request->AccountType)->get();
                        }
                        ...
                        if(count($check_already_fund_exists) > 0)
                        {
                            ...
                            {
                                $bank_profile = PlfBankProfile::find($request->plf_bank_id);
                            }
                            else{
                                $bank_profile = new PlfBankProfile();
                            }
                            
                            $bank_profile->name = $request->VONameasperPassbook;
                            $bank_profile->plf_id = $request->plf_id;
                            ...
                            $check_plf_data = PlfBankProfile::where('plf_id',$request->plf_id)->get();
                            if(count($check_plf_data) == 0)
                            {
                                ...
                                if($request->is_lawn1)
                                {
                                    PlfBankProfile::where('plf_id',$request->plf_id)->update(['is_primiary_account'=>0]);
                                    $bank_profile->is_primiary_account = 1;
                                }
                                else{
                            ...
                            if($request->PassbookPageUpload)
                            {
                                if ($uploadedFile = $this->getValidatedUpload('PassbookPageUpload', $request)) {
                                    $passbokk_url = $this->imageUploader->file($uploadedFile)
                                        ->savePassbookPicture($request->plf_id);
    ...
    public function getsc(PlfProfile $profile)
    { 
        dd($request->all());
        $ec = Ec::where('plf_id',$profile->id)->get();
        return response()->json(['status'=>true,"message"=>"Updated Successfully","ec"=>$ec]);

    }
    public function get_index_data(PlfProfile $profile)
    { 
        dd($request->all());
        $ec = Ec::where('plf_id',$profile->id)->get();
        return response()->json(['status'=>true,"message"=>"Updated Successfully","ec"=>$ec]);
    ...
    public function get_bank(PlfBankProfile $bankdetails)
    { 
    ...
    {
        $get_office_bearers = OfficeBearers::where('plf_id',$request->plf_id)->get();
        $get_ec_members = Ec::where('plf_id',$request->plf_id)->get();
    ...
    public function getAllUser(QueryFilterContract $queryFilter)
    {
        ...
        $get_panchat = Blocks::where('blockname',$user_data['block_name'])->first(); 
        $check_plf_profiles = PlfProfile::where('block',$get_panchat->id)->get(); 
        $query = PlfProfile::where('block',$get_panchat->id);
        $results = $queryFilter->builder($query)->customFilter(new CommonMasterFilter)->paginate(); 
        // dd($results);
        return view('admin.plfprofiles._user', compact('results'));
    }
    public function getbank(PlfProfile $profile,QueryFilterContract $queryFilter)
    {
        $url = request()->fullUrl(); //get full url
        $user_data = session('user_details'); 
        $check_plf_profiles = PlfBankProfile::where('plf_id',$profile->id)->get(); 
        $query = PlfBankProfile::where('plf_id',$profile->id); 
        $results = $queryFilter->builder($query)->customFilter(new CommonMasterFilter)->paginate(); 
        // dd($results[0]->Bankdetails);
        return view('admin.plfprofiles._banks', compact('results'));
    }
//searchUser
    ...
    public function indexDeleted()
    {
        ...
        return view('admin.plfprofiles.deleted-index', compact('page', 'main_menu', 'sub_menu','sub_menu_1'));
    }
    ...
    public function getAllDeletedUser(QueryFilterContract $queryFilter)
    {
        ...
        return view('admin.plfprofiles._deleted-user', compact('results'));
    }
...
/*inactive Users*/
    public function indexInactive()
    {
        ...
        return view('admin.plfprofiles.inactive-index', compact('page', 'main_menu', 'sub_menu','sub_menu_1'));
    }
    ...
    public function getAllInactiveUser(QueryFilterContract $queryFilter)
    {
        ...
        return view('admin.plfprofiles._inactive_user', compact('results'));
    }
/*end*/
    ...
    */
    public function create()
    {
        ...
        return view('admin.plfprofiles.create', compact('page', 'countries', 'main_menu', 'sub_menu','sub_menu_1'));
    }
    ...
    */
    ...
    public function getById(User $user)
    {
        ...
        return view('admin.plfprofiles.update', compact('page', 'countries', 'main_menu', 'results', 'sub_menu','sub_menu_1'));
    }
   ...
   public function UserTripRequestIndex(User $user)
    {
        ...
        return view('admin.plfprofiles.user-request-list-new', compact('card','main_menu','sub_menu','items','sub_menu_1'));
    }
    public function UserTripRequestNew(QueryFilterContract $queryFilter, User $user)
    {
        ...
        return view('admin.plfprofiles.user-request-list-view-new', compact('results', 'items'));
    }
    public function UserTripRequest(QueryFilterContract $queryFilter, User $user)
    {
        ...
        return view('admin.plfprofiles.user-request-list', compact('results','card','main_menu','sub_menu','sub_menu_1'));
    }
    public function UserTripRequestView(QueryFilterContract $queryFilter, User $user)
    {
        ...
        return view('admin.plfprofiles.user-request-list-view', compact('results','items'));
    }
    ...
    if ($amount == null) {
        ...
        return view('admin.plfprofiles.user-payment-wallet', compact('card','main_menu','sub_menu','item','history','sub_menu_1'));
    }
...
// dd($request);
    ...
    public function UserCancelRequestIndex(User $user)
    {
        ...
        return view('admin.plfprofiles.cancellation', compact('results', 'page', 'main_menu', 'sub_menu', 'user','sub_menu_1'));
    ...
    public function openbalance(){
        ...
        $main_menu = 'plf_profile';
        $sub_menu = 'plf_profile_open';
        $sub_menu_1 = 'plf_profile';
        $user_data = session('user_details');  
        $blocks_data = Blocks::where('blockname',$user_data['block_name'])->first();
        $plfprofile = PlfProfile::where('block',$blocks_data->id)->get(); 
        return view('admin.plfprofiles.openbalance', compact('plfprofile', 'page', 'main_menu', 'sub_menu','sub_menu_1'));
    }
    public function list_banks(PlfProfile $profile,QueryFilterContract $queryFilter){
        
        $query = PlfBankProfile::where('plf_id',$profile->id);
        $results = $queryFilter->builder($query)->customFilter(new CommonMasterFilter)->paginate();  
        return view('admin.plfprofiles._openbalance', compact('results'));
    }


    public function openbalanceedit(PlfBankProfile $profile)
    {
        $page = trans('pages_names.edit_user');
        ...
        $sub_menu = 'user_details';
        $sub_menu_1 = '';
        // print_r($profile->profile->PanchayatDetails->villagename);
        // exit;
        return view('admin.plfprofiles.update', compact('page', 'countries', 'main_menu', 'results', 'sub_menu','sub_menu_1'));
    }

    public function openbalanceupdate(PlfBankProfile $profile,Request $request)
    {
        $page = trans('pages_names.edit_user');
        // dd($request->all());
        ...
        if($request->front_copy)
        {
            if ($uploadedFile = $this->getValidatedUpload('front_copy', $request)) {
                $passbokk_url = $this->imageUploader->file($uploadedFile)
                    ->savePassbookPicture($profile->plf_id);
        ...
        return redirect('plf-profile/open-balance?panchayat='.$profile->profile->PanchayatDetails->id.'&profile='.$profile->plf_id.'')->with('success', 'Updated Successfully');
