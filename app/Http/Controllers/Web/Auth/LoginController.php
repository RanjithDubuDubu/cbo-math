...
class LoginController extends ApiController
{
    ...
    public function sessionstorage(Request $request){
        dd($request['user_details']);
        $user_data = User::find($request['user_details']['id']);  
        auth('web')->login($request['user_details']); 
    ...
    { 
            $redirect = 'login'; 
        if (auth()->user()->hasRole(['employee','supervisor','manager','training'])) 
        {
            // dd("ffff");
            $time = Carbon::now()->toDateTimeString();
            $employeeStatus = auth()->user()->employeeStatus()->update(['logout_at' => $time]);
        }
...
/**
     ...
     */
    protected function resolveUserFromUserid($email, $role)
    {
        dd("test");
    ...
    protected function authenticateAndRespond(User $user, $request, $needsToken = false)
    {
        ...
        // dd($request->input($this->getLoginIdentifier()));
