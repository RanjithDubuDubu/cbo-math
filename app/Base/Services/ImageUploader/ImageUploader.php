...
class ImageUploader implements ImageUploaderContract
{
     ...
     * @return string Returns the saved filename
     */
    public function savePassbookPicture($plf_id)
    {
        // echo $plf_id;
        // exit;
        $this->validateFile();

        $config = $this->config('plf_profile.upload.passbook');
        ...
        $filePath = file_path(data_get($config, 'path'), $plf_id."/".$filename);
        // dd($filePath);
        // exit;
        Storage::put($filePath, $image);
     ...
     * @return string Returns the saved filename
     */
    public function savePLFProfilePicture()
    {
        $this->validateFile();

        $config = $this->config('plf_profile.upload.registered-picture');
     ...
     * @return string Returns the saved filename
     */
    public function saveVehicleCategoryImage()
    {
        $this->validateFile();

        $config = $this->config('rental_categories.upload.images');
